  ...
  <v-container dark fluid fill-height>
    ...
    >
      ...
      <v-tabs dark class="rounded-0" show-arrows background-color="#1c1c22">
        <v-tab
          v-for="(item, index) in this.$store.state.calculator.DMGcalculator"
          :key="index"
        >
          {{ item.title }}
        </v-tab>
      ...
      <v-card-title class="justify-center text-truncate"
        >Special calculation formulas</v-card-title
      >
      <v-card dark tile elevation="0" color="#30303a">
        <Special @inputStat="getInput" />
      </v-card>
    </v-card>
  </v-container>
</template>
...
<script>
import AttrForm from "@/components/calculator/damage/in/attrDMGForm";
import SkillForm from "@/components/calculator/damage/in/SkillForm";
import Special from "@/components/calculator/damage/in/specialFormuls";
...
export default {
  ...
  components: { AttrForm, SkillForm, Special },
  methods: {
    getInput(input) {
      let inp = input.input;
      if (input.attr === "ATK") {
